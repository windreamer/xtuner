...
class MoE(BaseModel):
    ...
    def update_bias(
        ...
        # aligned with the rows of ``total_expert_counts_pre_iter`` / ``expected_loads``). Rows
        # resolving to the same physical gate (shared-weight MTP: one gate for several depth
        # rows) are aggregated first so that gate is updated once per step with the combined
        # load, instead of once per depth row.
        rows_by_layer: dict[str, list[int]] = {}
        for row_idx, layer_name in enumerate(layer_names):
            rows_by_layer.setdefault(layer_name, []).append(row_idx)

        row_index = torch.arange(len(layer_names), device=total_expert_counts_pre_iter.device, dtype=torch.long)
        for layer_name, row_idxs in rows_by_layer.items():
            ...
            if not isinstance(router, NoAuxRouter):
                continue
            rows = row_index[row_idxs]
            current_loads = total_expert_counts_pre_iter.index_select(0, rows).sum(dim=0)
            expected_load = expected_loads.index_select(0, rows).sum(dim=0)
